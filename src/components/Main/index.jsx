...
export default function Main(){
    ...
    return(
        <div className='dark:bg-lightDark'>
            <div className="flex flex-col items-center lg:flex-row min-w-full px-4 lg:px-72 py-20 min-h-[90vh]">
                <div className="lg:w-3/4 lg:order-1 text-center lg:text-left order-2">
                    {/*<h2 className="">Hello <span className=' text-xl'>👋</span> </h2>*/}
                    <h2 className="text-4xl md:text-5xl font-bold whitespace-nowrap mb-4 dark:text-white">Hey, <span className='text-violet-600'>I&apos;m Santhosh</span></h2>
            ...
            <div className='px-4 lg:px-72 pb-20'>
                <h2 className='text-2xl font-medium underline mb-4 dark:text-white'>Tools and Technologies</h2>
                <ul className='flex flex-wrap space-y-2 md:space-y-6 items-end'>
                    {icons.map((icon) => 
                    <li title={icon['title']} className=' mr-2 lg:mr-6 hover:scale-105 transition-transform hover:shadow-md ease-in-out flex items-center border dark:text-lightWhite rounded-md shadow-sm px-4 py-2 dark:border-0 dark:bg-gray-800' key={icon['title']}>
                        <img src={icon['imgUrl']} alt={icon['title']} className=' w-8 h-8' />
                        <span className='text-lg ml-4 hidden lg:block'>{icon['title']}</span>
                    </li>
                    )}
                </ul>
            </div>
            <div className='px-4 lg:px-72 pb-20'>
                <h2 className='text-2xl font-medium underline mb-4 dark:text-white'>Projects</h2>
                <ul className='spacing-y-4'>
                    {projects.map(project => <ProjectCard key={project.name} info={project}/>)}
                </ul>
            </div>
            <div className='px-4 lg:px-72 pb-20 dark:text-lightWhite'>
